...
const PropertyListing = () => {
  ...
  // Redux state access
  const { loading = false, propertydetails } = useSelector(
  ...
  const detailsToDisplay = propertydetails;

  
  if (loading || !detailsToDisplay || Object.keys(detailsToDisplay).length === 0) {
    return (
  ...
  // Address Formatting
  const formattedAddress = [address?.area, address?.city, address?.state]
    .filter(Boolean)
    .join(", ");
    ...
    <div className="property-container">
      ...
      <h6 className="property-location">
        ...
        <span className="location">
          {formattedAddress || "Address details unavailable"}
        </span>
      </h6>
