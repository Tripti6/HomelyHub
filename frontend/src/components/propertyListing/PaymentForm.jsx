...
import { setBookingDetails } from "../../../store/Booking/booking-slice";
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (name && guests && phoneNumber && checkinDate && checkoutDate) {
        ...
        dispatch(setBookingDetails(paymentDetails));
      ...
      >
        ...
        <div className="book-place">
          {!isAuthenticated ? (
            ...
            <button type="submit">Book this place &#8377; {calculatedPrice}</button>
          )}
        </div>
      </form>
    </div>
...
import { DatePicker, Space } from "antd";
import { useNavigate } from "react-router-dom";
import { useDispatch } from "react-redux"; // FIX 1: useDispatch import kiya
//import { setBookingDetails } from "../../store/Booking/booking-slice"; // FIX 2: 
import { setBookingDetails } from "../../../store/Booking/booking-slice";
...
const PaymentForm = ({
  ...
  const [calculatedPrice, setCalulatedPrice] = useState(0);
  const navigate = useNavigate();
  const dispatch = useDispatch(); // FIX 3: dispatch hook initialize kiya
  const { RangePicker } = DatePicker;
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (name && guests && phoneNumber && checkinDate && checkoutDate) {
        ...
        // FIX 4: User dwara bhari gayi details ko Redux Store me save kiya
        dispatch(setBookingDetails(paymentDetails));

        // Payment page par navigate kiya
        navigate(`/payment/${propertyId}`);
      } else {
        alert("Please fill all fields correctly before proceeding.");
      }
